...
if (oContainerPre) {
	if (oContainerPre.tagName == 'PRE'
			&& GetAttribute(oContainerPre, 'title') == 'code') {
		...
		if (CodeSettings.length > 0) {
			...
			if (CodeSettings.indexOf(";") > -1) {
				...
				oCodeSyntax.Lang = CodeSettings.substring(
						CodeSettings.indexOf(":") + 1,
						CodeSettings.indexOf(";")).replace(/^\s+|\s+$/g, "");
...
function Ok() {
	...
	if (GetE('chkHighlightLine').checked)
		advanced += "highlight: ["
				+ GetE('txtHighlightLines').value.replace(/\s/gi, "") + "]; ";
	...
	if (FCKBrowserInfo.IsIE) {
		oContainerPre.outerHTML = '<pre title="code" class="brush: ' + ddLang
