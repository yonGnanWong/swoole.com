...
class RenRenClient extends RESTClient{
    ...
    private $oauth_url = 'https://graph.renren.com/oauth/authorize';
    private $token_url = 'https://graph.renren.com/oauth/token';
    private $sess_url = 'https://graph.renren.com/renren_api/session_key';
    public $callback;
    ...
    function getAuthorizeURL($param=null)
    {
        $url = $this->oauth_url.'?response_type=code';
        $url.= "&client_id=".$this->_config->APIKey;
        $url.= "&redirect_uri=".urlencode($this->callback);
        if($param) foreach($param as $k=>$p) $url.="&$k=".urlencode($p);
        return $url;
    }
    function getToken($code)
    {
        $url = $this->token_url.'?grant_type=authorization_code&code='.$code;
        $url.= "&client_id=".$this->_config->APIKey;
        $url.= "&client_secret=".$this->_config->SecretKey;
        $url.= "&redirect_uri=".urlencode($this->callback);
        $context = stream_context_create(array('http'=>array('timeout'=>self::$socket_timeout)));
        return json_decode(file_get_contents($url,0,$context));
    }
    function getSession($token)
    {
        $url = $this->sess_url.'?oauth_token='.$token;
        $context = stream_context_create(array('http'=>array('timeout'=>self::$socket_timeout)));
        return json_decode(file_get_contents($url,0,$context));
    }
    /**
